...
Blockly.Blocks['block_enable'] = {
  init: function() {
    this.appendDummyInput().appendField('Enable Motion');
    this.setPreviousStatement(true, null);
    this.setNextStatement(true, null);
    this.setColour(120);
...
Blockly.Blocks['block_disable'] = {
  init: function() {
    this.appendDummyInput().appendField('Disable Motion');
    this.setPreviousStatement(true, null);
    this.setNextStatement(true, null);
    this.setColour(0);
...
Blockly.Blocks['block_if'] = {
  init: function() {
    ...
    this.appendStatementInput('condition').setCheck('String');
    this.setPreviousStatement(true, null);
    this.setNextStatement(true, null);
    this.setColour(90);
    this.setTooltip('If');
...
Blockly.Blocks['block_while'] = {
  init: function() {
    this.appendDummyInput()
      .appendField('While')
      .appendField(new Blockly.FieldTextInput('condition'), 'condition');
    this.appendStatementInput('condition').setCheck(null);
    this.setPreviousStatement(true, null);
    this.setNextStatement(true, null);
    this.setInputsInline(false);
    this.setColour(180);
    ...
    this.appendValueInput('varVal')
      ...
      .appendField(
        new Blockly.FieldDropdown([
          ...
          ['Decimal', 'double'],
          ['Text', 'string'],
          ['Joint', 'joint of xyzypr'],
          ['Location', 'location of xyzypr'],
        ]),
        'varType'
      );
    this.setPreviousStatement(true, null);
...
Blockly.Blocks['block_sub'] = {
  init: function() {
    ...
    this.appendStatementInput('routine').setCheck(null);
    this.setPreviousStatement(true, null);
    this.setNextStatement(true, null);
    this.setColour(210);
...
Blockly.Blocks['block_call'] = {
  init: function() {
    ...
    if (subs.length === 0) subs.push(['No Routines', 'undefined']);
    this.appendDummyInput()
...
Blockly.JavaScript['block_program'] = function(block) {
  ...
  var block_prefix = "'#" + block.id + '\n';
  if (block.forVarDeclaration) forVarDeclaration = 'Dim forVar as long\n';
  var code =
    block_prefix +
    'program\n' +
    forVarDeclaration +
    'Attach ' +
    Blockly.selectedRobot +
    '\n' +
    statements_program +
    '\nDetach ' +
    Blockly.selectedRobot +
...
Blockly.JavaScript['block_newvar'] = function(block) {
  var variable_varname = Blockly.JavaScript.variableDB_.getName(
    block.getFieldValue('varName'),
    Blockly.Variables.NAME_TYPE
  );
  var dropdown_vartype = block.getFieldValue('varType');
  var varVal = Blockly.JavaScript.valueToCode(
    block,
    'varVal',
  ...
  var forVarDeclaration = '';
  var prefix =
    block.getRootBlock().blockType !== undefined ? 'dim ' : 'dim shared ';
  var code = prefix + variable_varname + ' as ' + dropdown_vartype;
  if (varVal !== '') code += ' = ' + varVal;
  code += '\n';
  if (block.forVarDeclaration) forVarDeclaration = prefix + 'forVar as long\n';
...
Blockly.JavaScript['block_var_val'] = function(block) {
  ...
  var text_val = Blockly.JavaScript.valueToCode(
    block,
    'varVal',
    Blockly.JavaScript.ORDER_ATOMIC
  );
...
Blockly.JavaScript['block_move_var'] = function(block) {
  var text_position = Blockly.JavaScript.variableDB_.getName(
  ...
  var statements_parameters = Blockly.JavaScript.statementToCode(
    block,
    'Parameters'
  );
...
Blockly.JavaScript['block_move'] = function(block) {
  ...
  var position =
    text_position.indexOf(',') == -1
      ? text_position
      : pos_prefix + '{' + text_position + '}';
  var code =
    "'#" +
    block.id +
    '\n' +
