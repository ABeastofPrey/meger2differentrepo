/* ESSENTIAL WORKER CODE STARTS HERE */
"no use strict";
!(function(window) {
...
"use strict";
   ...
   function isCurrSubVar() {
    ...
    function startParsing() {
        ...
        for (let i=0; i<lines.length; i++) {
            ...
            if (token = isBlockEnd()) {
                if (token.err) {
                    error(token.err,idx,'error');
                    continue;
                }
                if (stack.length === 0) {
                    error('Unexpected END command',idx,'error');
                    continue;
                }
        ...
        if (stack.length) { // still unclosed tags
            for (let s of stack) {
    ...
    */
    function validateIfCommand() {
        ...
        if (match[7]) {
            const len  = match[7].length;
            if (len>0 && match[7][0] === "'") return null;
            if (len>2 && match[7].substring(0,3) === 'rem') return null;
            return new SyntaxError('Unexpected token after "then": '+ match[7],match,7);
        }
        return null;
    }
    
    function SyntaxError(msg, match, i) {
        this.msg = msg;
        this.col = indexOfGroup(match,i);
    }
   ...
   function validateForCommand(line) {
    ...
    function parseDim() {
        ...
        if (!parts[9]) {
            error(new SyntaxError('Expected variable type',parts,7),idx,'error');
            return;
        }
        ...
        if (v2) {
            ...
            error('Multiple definitions of the same variable', v2.line, 'error');
            return;
        }
        ...
        if (parts[10]) {
            if (!parts[11]) {
                error(new SyntaxError('Expected variable value',parts,10),idx,'error');
                return;
            }
            ...
            switch(varType) {
                case 'long':
                    ...
                    error(new SyntaxError(value + ' is not of type long',parts,11),idx,'error');
                    return;
            }
        }
    }

    function error(text, row, type) {
        if (typeof text === 'string')
            errors.push({row, text, type });
        else   
            errors.push({row, text: text.msg, type, col: text.col });
    }
    
});

/* ESSENTIAL WORKER CODE ENDS HERE */

/* CODE STARTS HERE */

define("ace/mode/mcbasic_worker",[], function(require, exports, module) {
"use strict";

var oop = require("../lib/oop");
var Mirror = require("../worker/mirror").Mirror;
var parser = require("./mcbasic/mcbasic_parse");

var MCBasicWorker = exports.Worker = function(sender) {
    Mirror.call(this, sender);
    this.setTimeout(1000);
};

oop.inherits(MCBasicWorker, Mirror);
