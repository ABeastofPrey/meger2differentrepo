...
export class BlocklyEditorComponent implements OnInit {
  ...
  private externalBlocksInit : boolean = false;
  ...
  private saveBlockColorState() {
    let blocks:any[] = this.workspace.getAllBlocks();
  ...
  initBlockly() {
    ...
    if (this.data.palletLibVer !== null) {
      let pallets = [];
      let robots = [];
      for (let p of this.data.pallets)
    ...
    if (this.data.gripperLibVer) {
      ...
      gripperPromise = this.ws.query('?grp_get_grippers_list').then((ret: MCQueryResponse)=>{
        for (let ef of ret.result.split(';')) {
          ...
          for (let grp of ef.substring(index + 1).split(',')) {
    ...
    gripperPromise.then(()=>{return this.loadBlocklyfiles();}).then(()=>{
  ...
  private getTrnErr() {
    this.ws.query('?TP_GET_TRNERR').then((ret: MCQueryResponse)=>{
      let trnerr = ret.result;
      var inULB = (trnerr.indexOf(".ULB") > -1);
      var inDEFVAR = (trnerr.indexOf(".DEF")>-1 || trnerr.indexOf(".VAR") > -1);
  ...
  onBlocklyChange(event : any) {
    ...
    this.getCode(true);
    var block = this.workspace.getBlockById(event.newValue);
  ...
  getCode(keepComments : boolean) {
    ...
    var blocklyCode = Blockly.JavaScript.workspaceToCode(this.workspace);
    /*this.blockLibs.forEach((libFile)=>{
      blocklyCode = 'import ' + libFile + '\n' + blocklyCode;
    });*/
    var lines = blocklyCode.split("\n");
    var i = lines.length;
    while (i--) {
  ...
  blocklyOnResize() {
    var element : any = this.blocklyArea;
