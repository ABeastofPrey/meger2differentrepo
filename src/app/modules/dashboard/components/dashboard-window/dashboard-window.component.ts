...
import {RecordDialogComponent, RecordParams} from '../record-dialog/record-dialog.component';
...
export class DashboardWindowComponent implements OnInit {
  ...
  move() {
    ...
    if (this.params.acc)
      cmd += ' Acc=' + this.params.acc;
    if (this.params.dec)
      cmd += ' Dec=' + this.params.dec;
    if (this.params.jerk)
  ...
  record() {
    ...
    ref.afterClosed().subscribe((params:RecordParams)=>{
      if (params) {
        ...
        let cmd = 
          'Record CSRECORD.rec ' +
          Math.ceil(params.duration) + ' Gap=1 RecData=' + varList.join();
        this.ws.query(cmd).then((ret: MCQueryResponse)=>{
          if (ret.err)
          ...
          this.ws.query('RecordOn').then((ret: MCQueryResponse)=>{
            ...
            let interval = setInterval(()=>{
              ...
              if (this.params.recordingTime >= 100) {
  ...
  onRecordFinish(is3D:boolean, isAdvanced : boolean) {
    this.stop();
    setTimeout(()=>{ // ALLOW TIME FOR RECORDING TO CLOSE FILE
