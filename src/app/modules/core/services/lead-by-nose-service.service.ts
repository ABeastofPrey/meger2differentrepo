...
export class LeadByNoseServiceService {
  ...
  set motionType(val: string) {
    ...
    this.ws.query(cmd).then((ret: MCQueryResponse) => {
      ...
      if (val === 'SINGLE_JOINT') this.joint = 1;
      else this._joint = null;
    });
  }
  ...
  set joint(val: number) {
    ...
    this.ws
      ...
      .then((ret: MCQueryResponse) => {
        if (ret.err || ret.result !== '0') this._joint = oldJoint;
      });
  }
  ...
  init() {
    this.ws
      ...
      .then((ret: MCQueryResponse) => {
        if (ret) this._vrate = Number(ret.result);
      });
  }
  ...
  reset() {
    if (this.keepAlive) this.ws.clearInterval(this.keepAlive);
    this._frames = [];
    this._frame = null;
    this._motionTypes = [];
    this._motionType = null;
