...
import { DATA_TYPE, InputType } from '../core/models/customKeyBoard/custom-key-board.model';
...
export class CustomKeyBoardComponent implements OnInit,OnDestroy,AfterViewInit {
    ...
    @Input() defaultValueOfInvalid = null;
    ...
    ngOnInit() {
        ...
        this.isIPAndMask && validators.push(UtilsService.ipAndMaskValidator());

        this.isUserName && validators.push(this.utils.validatorUserName());

        this.isNumberValue && validators.push(this.utils.precision(this.precision));
        ...
        this.ngZone.run(()=>{
            this.dialogRef = this.dialog.open(CustomKeyBoardDialogComponent, {
                data: {
                    ...
                    defaultValueOfInvalid: this.defaultValueOfInvalid
                },
                // width: '100vw',
                // height: '100vh',
                hasBackdrop: false
  ...
  public setInputValue(value: string, num: number): void {
    ...
    private emitBlurValue(value){
      ...
      if(this.control.status === 'INVALID' || this.control.value === ''){
        ...
        if(this.isNumberValue && this.defaultValueOfInvalid !== undefined){
          this.valueChange.emit(this.defaultValueOfInvalid);
          this.blurEvent.emit(this.defaultValueOfInvalid);
          return;
        }
      }
    ...
    private getErrorInfo(){
      ...
      if(index < 0){
        ...
        const errorCode = control.getError(errorCodes[index]);
    ...
    private getStringErrorCode(): string[]{//// sort by error severity level
      return [ErrorStringCode.Required,ErrorStringCode.MaxLength,ErrorStringCode.MinLength,ErrorStringCode.FirstLetter,ErrorStringCode.NameRules,ErrorStringCode.ExistNameList,ErrorStringCode.LetterAndNumber,ErrorStringCode.IPAndMask,
        ErrorStringCode.UserNameReserved,ErrorStringCode.InvalidUserName,ErrorStringCode.FullName,ErrorStringCode.ConfirmPassword]
    }
