...
export class ReferenceMasteringComponent implements OnInit, OnDestroy {
  ...
  public async cancel(): Promise<void> {
    this.trn
      .get([
        ...
        'robots.mastering.cancelWarm',
        'robots.mastering.cancelMsg',
        'button.yes',
        'button.cancel',
      ])
      .subscribe(words => {
  ...
  public selectionChange(event: MatSelectChange): void {
    ...
    const setReferencePoint = compose(
      then(logOrFetchAxisInfo),
      callApi
    );
  ...
  private async retrieveAndAssembleAxisInfo(): Promise<void> {
    ...
    const assembleTable = compose(
      detectChanges,
      bindToTable,
      mapIndexed(assembleRow)
    );
    const logOrAssemble = Either.either(logError, assembleTable);
    const _retrieveAndAssembleAxisInfo = compose(
      then(logOrAssemble),
      getAxisInfo
    );
  ...
  private async retrieveReferencePoints(): Promise<void> {
    const fetchReferencePoints = bind(
      this.service.fetchReferencePoints,
      this.service
    );
    const logError = err =>
      console.warn('Retrieve reference points failed: ' + err);
    ...
    const _retrieveReferencePoints = compose(
      then(logOrAssemble),
  ...
  private async retrieveSelectedPoint(): Promise<void> {
    const getReferencePoint = bind(
      this.service.getReferencePoint,
      this.service
    );
    const logError = err =>
      console.warn('Retrieve selected reference point failed: ' + err);
    const bindToAttr = point => (this.selectedPoint = point);
    const logOrAssemble = Either.either(logError, bindToAttr);
    const _retrieveSelectedPoint = compose(
      then(logOrAssemble),
      getReferencePoint
    );
  ...
  private async retrieveMoveCommand(): Promise<void> {
    ...
    const logError = err =>
      console.warn('Retrieve move command failed: ' + err);
    const bindToAttr = command => (this.moveCommand = command);
    const logOrAssemble = Either.either(logError, bindToAttr);
    const _retrieveMoveCommand = compose(
      then(logOrAssemble),
      getCommand
    );
