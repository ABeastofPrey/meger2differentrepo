...
export class CustomIOComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    this.ioService.queryCustomIos(this.tableIndex, this.customHex).then(() => {
      this.customDataSource.data = this.ioService.getCustomIos(this.customIOTypes);
      this.enableButtonsInCustomTab(true, false);
    });
  }
   ...
   */
  private selectRowInCustomTable(row, index) {
  ...
  private addRowInCustomTable() {
    ...
    if (defaultType) {
      ...
      this.ioService
        ...
        .then(() => {
          this.customDataSource.data[this.customDataSource.data.length - 1] = this.ioService.getCustomIo(this.customIOTypes);
   ...
   */
  private deleteRowInCustomTable() {
   ...
   */
  private removeSelectionOnPortOrTypeChange(event) {
   ...
   */
  private typeSelectionChange(index, value) {
    ...
    if (!select) {
      this.ioService
        ...
        .then(() => {
          this.customDataSource.data[index] = this.ioService.getCustomIo(this.customIOTypes);
          this.customTable.renderRows();
        });
      return;
    }
   ...
   */
  private portSelectionChange(index, value) {
   ...
   * @param event Onclick event on the row.
   */
  private onClickRadioButtonInCustomIO(index, event) {
    event.stopPropagation();
    this.changeSelectRow(null, -1);
    this.enableButtonsInCustomTab(true, false);
  ...
  private enableButtonsInCustomTab(isAddEnable: boolean, isDeleteEnable: boolean) {
    ...
    if (this.customDataSource.data.length > 99) {
