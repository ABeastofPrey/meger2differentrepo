...
describe('TopologyComponent', () => {
    ...
    const fakeService = jasmine.createSpyObj('TopologyService', ['getDeviceTopology', 'getOpMode']);
    let fakeMode = `OP
8`;
    let getDeviceTopologySpy = fakeService.getDeviceTopology.and.returnValue(Promise.resolve(Right(tree)));
    let getOpModeSpy = fakeService.getOpMode.and.returnValue(Promise.resolve(Right(fakeMode)));
...
2`;
    ...
    it('should do refresh after device topology is less than before.', async(() => {
        ...
        fixture.whenStable().then(() => {
            ...
            getDeviceTopologySpy = fakeService.getDeviceTopology.and.returnValue(Promise.resolve(Right(_tree)));
            expect(1).toBe(1);
        });
    }));
});
