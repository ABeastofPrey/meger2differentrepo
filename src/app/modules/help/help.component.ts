...
export class HelpComponent implements OnInit {
  ...
  public activation(): void {
    this.dialog.open(ActivationComponent, { disableClose: true, width: '250px' }).afterClosed().subscribe(
      machineId => {
        if (machineId) {
  ...
  private async getIdAndKey(): Promise<string[]> {
    ...
    const runErrIO = compose(IO.runIO, logErrIO);
    const logOrID = Either.either(runErrIO, identity);
    const logOrKey = Either.either(runErrIO, identity);
    const fetchID = compose(then(logOrID), getId);
    const fetchKey = compose(then(logOrKey), getKey);
    return await Promise.all([fetchID(), fetchKey()]);
  }

  private checkStatus(machineId: string, key: string): void {
    const hasIllegalValue = useWith(or, [isNilOrEmpty, isNilOrEmpty])(machineId, key);
    if (hasIllegalValue) { return; }
  ...
  private async saveHashKey(machineId: string): Promise<void> {
    ...
    const doIt = compose(then(logOrActivate), saveKey);
    doIt(hashCode);
  }
}
