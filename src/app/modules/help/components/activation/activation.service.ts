...
import { handler, errMsgProp } from '../../../../modules/program-editor/services/service-adjunct';
...
export class ActivationService {
    ...
    public async getMachineId(): Promise<any> {
        ...
        const retrieveID = compose(then(resHandler), query);
    ...
    public async getKey(): Promise<any> {
        ...
        const query = _api => this.ws.query(_api);
        const resHandler = handler(identity, errMsgProp);
        const retrieveKey = compose(then(resHandler), query);
    ...
    public async setKey(key: string): Promise<any> {
        ...
        const query = _api => this.ws.query(_api);
        const resHandler = handler(identity, errMsgProp);
        const saveKey = compose(then(resHandler), query);
        return saveKey(api);
    }
}
