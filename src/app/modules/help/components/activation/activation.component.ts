...
export class ActivationComponent implements OnInit {
  ...
  private retriveIDandEncryptRouteParameter(): void {
    ...
    const errIO = err =>
      IO(() => console.warn('Retrieve machine ID failed: ' + err));
    const runErrIO = compose(
      IO.runIO,
      errIO
    );
    const encrypt = compose(
      tap(assembleUrl),
      this.encryptRouteParameter.bind(this),
      tap(saveId)
    );
