...
export class PalletizingComponent implements OnInit {
  ...
  onWindowResize() {
    if (this.container) {
      ...
      this.preview.nativeElement.style.width = this.container.nativeElement.offsetWidth + 'px';
      this.preview.nativeElement.style.height = this.container.nativeElement.offsetHeight + 'px';
      this.drawPreview();
    }
  }
  ...
  addPallet() {
    ...
    ref.afterClosed().subscribe((ret:NewPalletOptions)=>{
      if (ret) {
        ...
        this.ws.query(
          '?PLT_ASSIGN_PALLET("' + name + '","' +
          ret.type + '",' + this.data.selectedRobot + ')'
      ...
      for (var i = pallet.items_x-1; i>=0; i--) {
        for (var j = max_y; j>=0; j--) {
          ...
          if (z < max_z || (pallet.items_x*j + i < countForThisLevel)) { // FULL FLOOR...
            result.push({
              shape:Shape.Prism(Point(x, y, this.adjustedMaxSize/10+z*itemZ), itemX - borderSize / 100, itemY - borderSize / 100, itemZ - borderSize / 100),
              z: z
            });
            count--;
          }
  ...
  private getPalletDrawingData():any{
    ...
    let pallet = this.data.selectedPallet;
    if (pallet === null || typeof pallet.index === 'undefined')
      return result;
