...
export class PalletLevelDesignerComponent implements OnInit {
  ...
  refresh() {
    ...
    if (pHeight / 270 > this.normalizeItem)
      this.normalizeItem = pHeight/270;
    if (pWidth / 550 > this.normalizeItem)
      this.normalizeItem = pWidth/550;
  ...
  validate(item: CustomPalletItem) { // CALLED AFTER EVERY DRAG
    item.untouched = false;
    let element : HTMLElement = item.element;
    let elementRect = element.getBoundingClientRect();
    let containerRect = (<HTMLElement> this.container.nativeElement).getBoundingClientRect();
    ...
    for (let other of this._items) {
      ...
      if (other === item)
        continue;
      let otherLeft = other.element.getBoundingClientRect().left - containerRect.left;
      let otherTop = other.element.getBoundingClientRect().top - containerRect.top;
    ...
    setTimeout(()=>{
      ...
      for (let i=1; i<this.container.nativeElement.childNodes.length; i++) {
        ...
        if (e.style && this.items[i-1]) {
          ...
          e.style.top = (
            containerRect.height - (this.items[i - 1].y / this.normalizeItem) -
