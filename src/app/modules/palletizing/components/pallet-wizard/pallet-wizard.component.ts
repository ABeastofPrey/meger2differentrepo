import { Component, OnInit, Inject, ViewChild, ElementRef, HostListener, Output } from '@angular/core';
import {MatSnackBar, MatDialog, MatSlideToggleChange, MatCheckboxChange, MatHorizontalStepper} from '@angular/material';
import {FormGroup, FormBuilder, Validators, AbstractControl} from '@angular/forms';
import {PalletLevelDesignerComponent} from '../pallet-level-designer/pallet-level-designer.component';
import {DataService, WebsocketService, ApiService, MCQueryResponse} from '../../../core';
...
export class PalletWizardComponent implements OnInit {
  ...
  private getPalletInfo() {
    ...
    let queries : Promise<any>[] = [
      ...
      this.ws.query('?PLT_GET_PALLETIZING_PRE_PLACE("'+name+'","VERTICAL")'),
      this.ws.query('?PLT_GET_PALLETIZING_PRE_PLACE("'+name+'","HORIZONTAL")'),
      this.ws.query('?PLT_GET_PALLETIZING_POST_PLACE("'+name+'","VERTICAL")'),
      this.ws.query('?PLT_GET_PALLETIZING_POST_PLACE("'+name+'","HORIZONTAL")'),
    ...
    return Promise.all(queries).then((ret:MCQueryResponse[])=>{
      ...
      let itemCount=(ret[1].result.length===0?'0,0,0':ret[1].result).split(',');
      ...
      this.dataService.selectedPallet.entry = this.parseLocation(ret[11].result);
      ...
      this.dataService.selectedPallet.approach_offset_vertical = Number(ret[14].result);
      this.dataService.selectedPallet.approach_offset_horizontal = Number(ret[15].result);
      this.dataService.selectedPallet.retract_offset_vertical = Number(ret[16].result);
      this.dataService.selectedPallet.retract_offset_horizontal = Number(ret[17].result);
      ...
      this.dataService.selectedPallet.index = Number(ret[21].result) || 0;
      this.dataService.selectedPallet.appExceed = (ret[22].result === '1');
      this.dataService.selectedPallet.retExceed = (ret[23].result === '1');
      let palletSizes=(ret[24].result.length===0?'0,0,0':ret[24].result).split(',');
      ...
      this.toggleRetEnable(new MatSlideToggleChange(null, this.dataService.selectedPallet.retEnabled),true);
      this.toggleAppEnable(new MatSlideToggleChange(null, this.dataService.selectedPallet.appEnabled),true);
      this.toggleEntryEnable(new MatSlideToggleChange(null, this.dataService.selectedPallet.entryEnabled),true);
      this.toggleAppExceed({source: null, checked:this.dataService.selectedPallet.appExceed},true);
      this.toggleRetExceed({source: null, checked:this.dataService.selectedPallet.retExceed},true);
      this.dataService.selectedPallet.dataFile = ret[25].err ? null : ret[25].result;
  ...
  calibrate() {
    this.ws.query(
      '?PLT_ORIGIN_CALIBRATION("' + this.dataService.selectedPallet.name + '")'
    ).then((ret: MCQueryResponse)=>{
      this.dataService.selectedPallet.isFrameCalibrated = (ret.result === '0');
      if (ret.result !== '0') {
        this.ws.query('?PLT_GET_ERROR').then((err: MCQueryResponse)=>{
          this.snack.open(err.result,'',{duration: 2000});
        });
      }
    });
  }
  ...
  onIndexChange(val?: number) {
    if (val)
      this.dataService.selectedPallet.index = val;
    this.step1.controls['index'].setValue(this.itemIndexMode);
  }
  ...
  private getItemsXYZ(itemX:number, itemY:number, itemZ: number ) {
    ...
    for (var z = 0; z<=max_z; z++) {
      ...
      var max_y = z < max_z ? pallet.items_y-1 : Math.floor((countForThisLevel-1)/pallet.items_x);
      for (var i = pallet.items_x-1; i>=0; i--) {
        for (var j = max_y; j>=0; j--) {
          var x = i * itemX;
          var y = j * itemY;
          if (z < max_z || (pallet.items_x*j + i < countForThisLevel)) { // FULL FLOOR...
            result.push({
              shape:Shape.Prism(Point(x, y, 0.4+z*itemZ), itemX - borderSize / 100, itemY - borderSize / 100, itemZ - borderSize / 100),
              z: z
            });
            count--;
          }
  ...
  private getItemsYXZ(itemX:number, itemY:number, itemZ: number ) {
    ...
    for (var z = 0; z<=max_z; z++) {
      var countForThisLevel = count;
      var max_x = z < max_z ? pallet.items_x-1 : Math.floor((countForThisLevel-1)/pallet.items_y);
      for (var i = max_x; i>=0; i--) {
        for (var j = pallet.items_y-1; j>=0; j--) {
          var x = i * itemX;
          var y = j * itemY;
          if (z < max_z || (pallet.items_y*i + j < countForThisLevel)) { // FULL FLOOR...
            result.push({
              shape:Shape.Prism(Point(x, y, 0.4+z*itemZ), itemX - borderSize / 100, itemY - borderSize / 100, itemZ - borderSize / 100),
              z: z
            });
  ...
  private getPalletDrawingData():any{
    ...
    let pallet = this.dataService.selectedPallet;
    if (typeof pallet.index === 'undefined')
      return result;
    ...
    if (pallet.items_x > 0 && pallet.items_y > 0 && pallet.items_z > 0 && pallet.item_size_x > 0 && pallet.item_size_y > 0 && pallet.item_size_z > 0) {
  ...
  private validateOrder(control : AbstractControl) {
    ...
    var cmd =
      ...
      this.dataService.selectedPallet.name + '","' + control.value + '")';
    return this.ws.query(cmd).then((ret: MCQueryResponse)=>{
  ...
  private validateLevels(control:AbstractControl) {
    ...
    let cmd = '?PLT_SET_NUMBER_OF_LEVELS("' + pallet + '",'+control.value + ')';
    return this.ws.query(cmd).then((ret: MCQueryResponse)=>{
  ...
  private validateItems(changed: string, control: AbstractControl) {
    ...
    if (x && y && z) {
      var cmd =
        '?PLT_SET_NUMBER_OF_ITEMS("' + pallet.name + '",' +
        x + ',' + y + ',' + z + ')';
      return this.ws.query(cmd).then((ret: MCQueryResponse)=>{
        this.onIndexChange();
  ...
  private validateItemSizeX(control : AbstractControl) {
    var pallet = this.dataService.selectedPallet;
    var cmd =
      '?PLT_SET_ITEM_DIMENSION("' + pallet.name + '","X",'+control.value + ')';
    return this.ws.query(cmd).then((ret: MCQueryResponse)=>{
  ...
  private validateItemSizeY(control : AbstractControl) {
    var pallet = this.dataService.selectedPallet;
    var cmd =
      '?PLT_SET_ITEM_DIMENSION("' + pallet.name + '","Y",'+control.value + ')';
    return this.ws.query(cmd).then((ret: MCQueryResponse)=>{
      return (ret.err || ret.result !== '0') ? { invalidSizeX: true } : null;
    });
  }
  ...
  private validateItemSizeZ(control : AbstractControl) {
    var pallet = this.dataService.selectedPallet;
    var cmd =
      '?PLT_SET_ITEM_DIMENSION("' + pallet.name + '","Z",'+control.value + ')';
    return this.ws.query(cmd).then((ret: MCQueryResponse)=>{
      return (ret.err || ret.result !== '0') ? { invalidSizeX: true } : null;
    });
  }
  ...
  private validatePalletSize(changed:string, control:AbstractControl) {
    let pallet = this.dataService.selectedPallet.name;
    let x = (changed === 'x') ? 
            control.value : this.dataService.selectedPallet.pallet_size_x;
    let y = (changed === 'y') ?
            control.value : this.dataService.selectedPallet.pallet_size_y;
    let z = (changed === 'z') ? 
            control.value : this.dataService.selectedPallet.pallet_size_z;
    if (x && y && z) {
      if (!control.touched && !control.dirty)
        return Promise.resolve(null);
      var cmd = '?PLT_SET_PALLET_SIZE("'+pallet+'",'+x+','+y+','+z+')';
      return this.ws.query(cmd).then((ret: MCQueryResponse)=>{
        if (ret.err || ret.result !== '0') {
  ...
  private validateEntry(changed: string, control: AbstractControl) {
    ...
    if (x !== null && y !== null && z !== null &&
      ...
      var cmd =
        ...
        this.dataService.selectedPallet.name + '",' + loc + ')';
      return this.ws.query(cmd).then((ret: MCQueryResponse)=>{
        if (ret.err || ret.result !== '0') {
  ...
  private validateAppOffsetV(control: AbstractControl) {
    var pallet = this.dataService.selectedPallet;
    if (!pallet.appEnabled)
      return Promise.resolve(null);
    var off = control.value;
    var cmd = '?PLT_SET_PALLETIZING_PRE_PLACE("' + pallet.name + '","VERTICAL",'+off+')';
    return this.ws.query(cmd).then((ret: MCQueryResponse)=>{
      if (ret.err || ret.result !== '0') {
  ...
  private validateAppOffsetH(control: AbstractControl) {
    ...
    if (!pallet.appEnabled)
      return Promise.resolve(null);
    var off = control.value;
    var cmd = '?PLT_SET_PALLETIZING_PRE_PLACE("' + pallet.name + '","HORIZONTAL",'+off+')';
    return this.ws.query(cmd).then((ret: MCQueryResponse)=>{
      if (ret.err || ret.result !== '0') {
  ...
  private validateAppDir(control: AbstractControl) {
    ...
    var cmd = '?PLT_SET_HANDLING_DIRECTION("' + pallet.name + '","APPROACH","'+dir+'")';
    return this.ws.query(cmd).then((ret: MCQueryResponse)=>{
      if (ret.err || ret.result !== '0') {
  ...
  private validateRetOffsetV(control: AbstractControl) {
    var pallet = this.dataService.selectedPallet;
    if (!pallet.retEnabled)
      return Promise.resolve(null);
    var off = control.value;
    var cmd = '?PLT_SET_PALLETIZING_POST_PLACE("' + pallet.name + '","VERTICAL",'+off+')';
    return this.ws.query(cmd).then((ret: MCQueryResponse)=>{
      if (ret.err || ret.result !== '0') {
  ...
  private validateRetOffsetH(control: AbstractControl) {
    ...
    if (!pallet.retEnabled)
      return Promise.resolve(null);
    var off = control.value;
    var cmd = '?PLT_SET_PALLETIZING_POST_PLACE("' + pallet.name + '","HORIZONTAL",'+off+')';
    return this.ws.query(cmd).then((ret: MCQueryResponse)=>{
      if (ret.err || ret.result !== '0') {
  ...
  private validateRetDir(control: AbstractControl) {
    ...
    if (!pallet.retEnabled)
      return Promise.resolve(null);
    var dir = control.value;
    var cmd = '?PLT_SET_HANDLING_DIRECTION("' + pallet.name + '","RETRACT","'+dir+'")';
    return this.ws.query(cmd).then((ret: MCQueryResponse)=>{
      if (ret.err || ret.result !== '0') {
  ...
  finish() {
    ...
    this.ws.query('?PLT_ENABLE_PALLET("' + name + '")').then((ret: MCQueryResponse)=>{
      if (ret.result === '0') {
        this.closeDialog(false);
      }
      else {
        this.ws.query('?PLT_GET_ERROR').then((ret: MCQueryResponse)=>{
          this.lastError = ret.result;
        });
      }
    });
  }
  ...
  ngOnInit() {
    ...
    this.step1 = this._formBuilder.group({
      order: pallet.type === 'CUSTOM' ? [''] : ['', [Validators.required], this.validateOrder.bind(this)],
      itemsX: pallet.type === 'CUSTOM' ? [''] : ['', [Validators.required, Validators.min(1), Validators.max(1000)], this.validateItems.bind(this,'x')],
      itemsY: pallet.type === 'CUSTOM' ? [''] : ['', [Validators.required, Validators.min(1), Validators.max(1000)], this.validateItems.bind(this,'y')],
      itemsZ: pallet.type === 'CUSTOM' ? [''] : ['', [Validators.required, Validators.min(1), Validators.max(1000)], this.validateItems.bind(this,'z')],
      itemSizeX: ['', [Validators.required, Validators.min(1)], this.validateItemSizeX.bind(this)],
      itemSizeY: ['', [Validators.required, Validators.min(1)], this.validateItemSizeY.bind(this)],
      itemSizeZ: ['', [Validators.required, Validators.min(1)], this.validateItemSizeZ.bind(this)],
      palletSizeX: ['', [Validators.required, Validators.min(10)], this.validatePalletSize.bind(this,'x')],
      palletSizeY: ['', [Validators.required, Validators.min(10)], this.validatePalletSize.bind(this,'y')],
      palletSizeZ: ['', [Validators.required, Validators.min(10)], this.validatePalletSize.bind(this,'z')],
      index: ['',[], this.validateIndex.bind(this)],
      levels: pallet.type === 'GRID' ? [''] : ['', [Validators.required, Validators.min(1)], this.validateLevels.bind(this)],
      diffOddEven: ['']
    });
    this.step2 = this._formBuilder.group({
      ...
      calibrated: [false, Validators.requiredTrue, this.checkCalibrationStatus.bind(this)]
    });
    this.step3 = this._formBuilder.group({
      ...
      pitch: ['', [Validators.required], this.validateEntry.bind(this,'pitch')],
      roll: ['', [Validators.required], this.validateEntry.bind(this,'roll')],
      entryEnable: ['',[]]
    });
    this.step4 = this._formBuilder.group({
      app_off_v: ['', [Validators.required,Validators.min(0)], this.validateAppOffsetV.bind(this)],
      app_off_h: ['', [Validators.required,Validators.min(0)], this.validateAppOffsetH.bind(this)],
      app_dir: ['', [Validators.required], this.validateAppDir.bind(this)],
      ret_off_v: ['', [Validators.required,Validators.min(0)], this.validateRetOffsetV.bind(this)],
      ret_off_h: ['', [Validators.required,Validators.min(0)], this.validateRetOffsetH.bind(this)],
    ...
    this.stepCustom1 = this._formBuilder.group({
      levelExists: ['', [Validators.required,Validators.min(1)], this.validateLevel.bind(this,1)]
    });
    this.stepCustom2 = this._formBuilder.group({
      levelExists: ['', [Validators.required,Validators.min(1)], this.validateLevel.bind(this,2)]
    });
  }
  
  onLevelChange(i:number, itemCount: number) {
    if (i===1)
      this.stepCustom1.controls['levelExists'].setValue(itemCount);
    else
      this.stepCustom2.controls['levelExists'].setValue(itemCount);
  }
  ...
  toggleRetExceed(e : MatCheckboxChange, noQuery? : boolean) {
    ...
    let name = this.dataService.selectedPallet.name;
    let cmd = '?PLT_ENABLE_POST_PLACE("'+name+'",'+retEnabled+','+val+')';
    this.ws.query(cmd).then((ret: MCQueryResponse)=>{
      if (ret.result !== '0' || ret.err) {
  ...
  toggleRetEnable(e : MatSlideToggleChange, noQuery? : boolean) {
    ...
    let cmd = '?PLT_ENABLE_POST_PLACE("'+name+ '",' + val + ',' + exceed + ')';
    this.ws.query(cmd).then((ret: MCQueryResponse)=>{
      if (ret.result !== '0' || ret.err) {
  ...
  toggleAppExceed(e : MatCheckboxChange, noQuery? : boolean) {
    ...
    let name = this.dataService.selectedPallet.name;
    let cmd = '?PLT_ENABLE_PRE_PLACE("'+name+'",'+enabled+','+val+')';
    this.ws.query(cmd).then((ret: MCQueryResponse)=>{
      if (ret.result !== '0' || ret.err) {
  ...
  toggleAppEnable(e:MatSlideToggleChange,noQuery? : boolean) {
    ...
    let cmd = '?PLT_ENABLE_PRE_PLACE("'+name+ '",' + val + ',' + exceed + ')';
    this.ws.query(cmd).then((ret: MCQueryResponse)=>{
      if (ret.result !== '0' || ret.err) {
  ...
  toggleEntryEnable(e:MatSlideToggleChange,noQuery? : boolean) {
    ...
    var name = this.dataService.selectedPallet.name;
    this.ws.query('?PLT_ENABLE_ENTRY_POSITION("' + name + '",' + val + ')')
    .then((ret: MCQueryResponse)=>{
      if (ret.result !== '0' || ret.err) {
  ...
  onWindowResize() {
    ...
    this.preview1.nativeElement.style.width = this.container.nativeElement.offsetWidth - 2 + 'px';
    this.preview1.nativeElement.style.height = this.container.nativeElement.offsetHeight - 2 + 'px';
    this.drawPreview1();
  }
  
  drawPreview1() {
    if (!this.step1.dirty && this.isPreview1Init)
      return;
    // clear canvas
