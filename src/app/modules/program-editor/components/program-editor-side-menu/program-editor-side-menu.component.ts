...
export class ProgramEditorSideMenuComponent implements OnInit {
  ...
  constructor(
    ...
    this.trn.get([
      ...
      'button.cancel','projectTree.del_app_msg','projectTree.del_lib_msg',
      'projects.toolbar.rename','projects.toolbar.app_name','button.rename',
    ...
    this.trn.get('projectTree.del_app_title', {name: app}).subscribe(word=>{
      this.dialog.open(YesNoDialogComponent,{
        ...
        if (ret) {
          ...
          this.ws.query('?prj_remove_app("' + prj.name + '","' + app + '")').then((ret:MCQueryResponse)=>{
            if (ret.result === '0') {
              this.data.refreshDomains().then(()=>this.prj.refreshAppList(prj,true))
              .then(ret=>{
    ...
    this.trn.get('projectTree.del_lib_title', {name: lib}).subscribe(word=>{
      this.dialog.open(YesNoDialogComponent,{
        ...
        if (ret) {
          ...
          const cmd = '?prj_remove_app_library("' + prj.name + '","' + app + '","' + 
                      lib + '")';
          this.ws.query(cmd).then((ret:MCQueryResponse)=>{
            if (ret.result === '0') {
    ...
    setTimeout(()=>{
      ...
      if (appsElement) {
        ...
        list.dropped.subscribe(e=>{
          let data = this.nestedDataSource.data;
          data[0].children.splice(e.currentIndex,0,data[0].children.splice(e.previousIndex,1)[0]);
