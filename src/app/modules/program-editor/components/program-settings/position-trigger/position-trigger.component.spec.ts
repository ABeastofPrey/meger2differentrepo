import { PositionTriggerComponent, IPositionTrigger, initColumns, assemblePLS, assemblePLSList,
    isChecked, isAllChecked, notAllChecked, hasChecked, partialChecked, checkedLensProp,
    checkElement, uncheckElement, checkAll, uncheckAll, toggleAll, canCreate, isNumber,
    isPositiveNumber, isChanged, getSelectedNames, insertComma, combineNames
...
describe('PositionTriggerComponent', () => {
    ...
    const count = 5;
    const name = 'SSS';
    const resPls: IResPLS = {index: 1, PLSname: name, DigitalOut: 1, Position: 1, Polarity: 0, RelatedTo: 0, Output: [1, 2, 3]};
    ...
    const partialCheckedObjList = map((n) => (n % 2 === 0) ? checkedObj : uncheckedObj, range(0, 5));
    const hasNoChecked = all(compose(not, isChecked));

    const fakeService = jasmine.createSpyObj('MotionTriggerService', ['retrievePls', 'updatePls', 'deletePls', 'createPls']);
    let retrievePlsSyp = fakeService.retrievePls.and.returnValue(Right(resPlsList));
    const updatePlsSpy = fakeService.updatePls.and.returnValue(Left('update failed.'));
    const deletePlsSpy = fakeService.deletePls.and.returnValue(Left('delete failed.'));
    ...
    it('should assemble data failed after on-init.', async(() => {
        retrievePlsSyp = fakeService.retrievePls.and.returnValue(Left('retrieve failed.'));
        fixture.detectChanges();
        expect(retrievePlsSyp.calls.any()).toBe(true, 'retrievePls be called.');
    ...
    it('should create pls when gived corrected name.', () => {
        ...
        fakeMatDialog.open().afterClosed().emit('mypls');
