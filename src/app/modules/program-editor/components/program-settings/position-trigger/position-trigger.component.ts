...
export interface IPositionTrigger {
    ...
    output: number[]; state: number[]; from: number[];
    selectedOutput: number; selectedState: number; selectedFrom: number;
}

type JudgeCheckFn = (elements: IPositionTrigger[]) => boolean;
type CheckElesFn = (elements: IPositionTrigger[]) => IPositionTrigger[];
type ToggleAllFn = (element: { checked: boolean }) => (elements: IPositionTrigger[]) => IPositionTrigger[];

const { Nothing } = Maybe;

export const freeze = x => Object.freeze(x);

export const initColumns: () => ReadonlyArray<string> = always(freeze(['check', 'Name', 'Output', 'State', 'From', 'Distance']));

export const assemblePLS: (x: IResPLS) => IPositionTrigger = x => {
    const obj: IPositionTrigger = {
        checked: false, name: x.PLSname, output: x.Output,
        distance: x.Position, state: [0, 1], from: [0, 1],   // for from: 0 means start point, 1 means end point.
        selectedOutput: x.DigitalOut, selectedState: x.Polarity, selectedFrom: x.RelatedTo
    };
    return obj;
};
...
export class PositionTriggerComponent implements OnInit, OnDestroy {
    ...
    public async deletePls(): Promise<void> {
        ...
        const deleteAll = await Promise.all(map(bindDelete, getSelectedNames(this.data)));
    ...
    public onFocus(distance: number) { this.preDistance = distance; }
    ...
    private async assembleData(): Promise<void> {
        Either.either(
            err => console.log(err), // handle error.
            (res: IResPLS[]) => { this.data = assemblePLSList(res); } // bind data.
