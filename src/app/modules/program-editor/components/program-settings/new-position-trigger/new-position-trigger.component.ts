...
import { isNil, or, gte, lte, converge, and, __, equals, anyPass, length, compose } from 'ramda';
...
const isNumber: (keyCode: number) => boolean = converge(or, [converge(and, [gte48, lte57]), converge(and, [gte96, lte105])]);
const isBackSpace: (keyCode: number) => boolean = equals(8);
const isOverLimit: (x: string) => boolean = compose(gte24, length);
const isLegalInput: (keyCode: number) => boolean = anyPass([isBackSpace, isLetter, isNumber]);
...
export class NewPositionTriggerComponent implements OnInit {
    ...
    public get canNotCreate(): boolean { return or(isNil(this.name), this.nameControl.hasError('required')); }

    ngOnInit(): void { }

    public close(): void { this.dialogRef.close(this.namePrefix + this.name); }

    public onKeyDown(keyCode: number): boolean {
        if (isOverLimit(this.name)) {
            if (isBackSpace(keyCode)) { return true; }
            return false;
        }
